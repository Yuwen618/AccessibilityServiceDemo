...
public class DragListView extends ListView {
    ...
    private static final int MSG_START_DRAG = 1001;
    private static final int TIME_DELAY_TO_STARTDRAG = 500;
    ...
    private void init() {
        mHandler = new Handler(new DragHandler());
    }
    private class DragHandler implements Handler.Callback {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                Log.d("DRAGLISTVIEW", "action down..");
                mDownX = (int) ev.getX();
                mDownY = (int) ev.getY();
                // 获取当前触摸位置对应的条目索引
                ...
                mSwapped = false;
                mHandler.removeMessages(MSG_START_DRAG);
                mHandler.sendEmptyMessageDelayed(MSG_START_DRAG, TIME_DELAY_TO_STARTDRAG);
                // 开始拖拽的前期工作：展示item快照
                //startDrag();
                Log.d("DRAGLISTVIEW", "start drag..");
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsDraging) {
                    ...
                    // 更新当前被替换的位置
                    updateItemView();
                    Log.d("DRAGLISTVIEW", "update drag..");
                } else {
                    Log.d("DRAGLISTVIEW", "return 2..");
                    return super.onTouchEvent(ev);
                }
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mIsDraging) {
                    // 停止拖拽
                    stopDrag();
                    Log.d("DRAGLISTVIEW", "stop drag..");
                } else {
                    int dx = mDownX - mEndX;
                    int dy = mDownY - mEndY;
                    if (((dx*dx + dy*dy)<10) && mDragPosition != AdapterView.INVALID_POSITION) {
                        performItemClick(this, mDragPosition, 0);
                        return true;
                    }
                    Log.d("DRAGLISTVIEW", "return 3..");
                    return super.onTouchEvent(ev);
                }
                break;
            default:
    ...
    private void createDragPhotoView() {
        ...
        mWindowLayoutParams.y += 10;//show at difference position to let user know

        mDragPhotoView = new ImageView(getContext());
        mDragPhotoView.setImageBitmap(mDragPhotoBitmap);
        mDragPhotoView.setBackgroundResource(R.drawable.border);
